...
addTask(){
  ...
  this.http.post(`${this.api_url}/tasks`,bodyData).subscribe(
    (res:any)=>{
...
getTasks() {
  this.http.get(`${this.api_url}/tasks`)
...
deleteTask(data: any)
{
    ...
    if(confirmDelete){
      this.http.delete(`${this.api_url}/tasks`+ "/"+ data.id).subscribe((resultData: any)=>
        {
            console.log(resultData);
            alert("Deleted")
            this.getTasks();
...
UpdateRecords()
  {
    ...
    this.http.put(`${this.api_url}/tasks/`+ this.currenttaskid , bodyData).subscribe((resultData: any)=>
    {
        console.log(resultData);
        this.task_name = '';
        this.task_description = '';
